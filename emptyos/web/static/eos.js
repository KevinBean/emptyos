...
(function() {
    ...
    // Auto-mount nav: any page can opt-in by including <div id="eos-nav"></div>.
    // Page id derives from the URL prefix (which equals the app id by convention).
    // Pages that already call EOS.nav('foo') explicitly win — this is a fallback.
    function _autoMountNav() {
        if (EOS._currentApp) return;
        if (!document.getElementById('eos-nav')) return;
        var seg = (location.pathname.split('/')[1] || '').toLowerCase();
        if (!seg || seg === 'static' || seg === 'api' || seg === 'docs' || seg === 'ws') return;
        EOS.nav(seg || 'home');
    }
    ...
    EOS._whenAppsKnown(function() {
        ...
        var auraFab = document.createElement('div');
        auraFab.title = 'EmptyOS tools';
        auraFab.setAttribute('role', 'button');
        auraFab.setAttribute('aria-label', 'Open tools menu');
        auraFab.style.cssText = 'width:48px;height:48px;border-radius:50%;background:linear-gradient(135deg, #00f2fe, #4facfe);box-shadow:0 0 20px rgba(0, 242, 254, 0.4), inset 0 0 10px rgba(255,255,255,0.3);display:flex;align-items:center;justify-content:center;color:white;cursor:pointer;transition:all 0.3s cubic-bezier(0.175, 0.885, 0.32, 1.275);';
        auraFab.innerHTML = '<span style="font-size:22px; text-shadow: 0 2px 4px rgba(0,0,0,0.2);">✨</span>';
        
        var animStyle = document.createElement('style');
        animStyle.textContent = 
            '@keyframes aura-float { 0%, 100% { transform: translateY(0) scale(1); } 50% { transform: translateY(-3px) scale(1.05); } } ' +
            '.eos-fab-hidden { opacity: 0; pointer-events: none; transform: translateY(15px) scale(0.9); } ' +
        ...
        auraFab.style.animation = 'aura-float 3s ease-in-out infinite';
        ...
        dock.onmouseenter = function() {
            ...
            auraFab.style.transform = 'scale(1.1)';
            auraFab.style.boxShadow = '0 0 30px rgba(0, 242, 254, 0.6)';
        };
        dock.onmouseleave = function() {
            hoverTimeout = setTimeout(function() {
                othersContainer.className = 'eos-fab-hidden';
                auraFab.style.transform = '';
                auraFab.style.boxShadow = '0 0 20px rgba(0, 242, 254, 0.4)';
