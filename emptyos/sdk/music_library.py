"""Shared song & album catalogue for music apps.

Used by `apps/music-library/` (community) and `apps/personal/music-studio/`
(personal, ComfyUI-driven generation). Both apps want the same vault-tag
query surface + filesystem helpers (audio file resolution, cover art
discovery, copyright screenshot scanning); they differ only in what they
expose over HTTP.

When NOT to use:
- Apps that aren't catalogues over `tag: song` / `tag: album` notes.
- Apps that need a different field set or sort order — subclass
  `SongLibrary` / `AlbumLibrary` instead of forking the mixin.
"""
...
SUGGEST_NEXT_SYSTEM = """You are a music-library coach helping the user pick the next song to work on.

Pick exactly ONE song from the list and explain in 2 sentences why it should be next. Reasoning should weigh:
- Drafts and in-progress beat finished/published — those need work, finished ones don't.
- Stale entries (no recent attention) over freshly-touched ones.
- Variety in genre/mood when several candidates tie.

Do NOT:
- Suggest more than one song.
- List multiple candidates "in case the user disagrees" — pick one.
- Recommend songs whose status is "finished", "published", or "released" unless every other song is also finished.
- Pad with greetings, emojis, hedging ("maybe you could…"), or meta commentary about the list.
- Quote the input back at the user.
"""

SUGGEST_NEXT_USER_TMPL = "Songs:\n{lines}\n\nSuggest the next one to work on."
...
class SongLibrary(VaultLibrary):
    ...
    fields = {
        ...
        "created": str,
        "published": str,
        # External-source provenance — exposed raw so source panels can render
        # service-specific links (Open in Suno / YouTube). Without them in
        # `fields`, VaultLibrary strips them from `detail()` output.
        "source_url": str,
        "suno_url": str,
        "suno_model": str,
        "youtube_url": str,
...
class LibraryMixin:
    # Subclasses may override these to use customised SongLibrary/AlbumLibrary.
    song_library_cls: type[VaultLibrary] = SongLibrary
    album_library_cls: type[VaultLibrary] = AlbumLibrary

    def __init__(self, app: BaseApp):
        self.app = app
        extra = app.app_config("extra_fields", [])
        self.songs = self.song_library_cls(app, extra_fields=extra or None)
        self.albums = self.album_library_cls(app)
    ...
    def _enrich(self, items: list[dict]) -> list[dict]:
        ...
        for s in items:
            ...
            prov_provider = (
                (s.get("ai_provider") or "").strip()
                if isinstance(s.get("ai_provider"), str)
                else ""
            )
            prov_model = (
                (s.get("ai_model") or "").strip() if isinstance(s.get("ai_model"), str) else ""
            )
    ...
    async def suggest_next(self) -> dict:
        ...
        result = await self.app.think(
            SUGGEST_NEXT_USER_TMPL.format(lines="\n".join(lines)),
            domain="text",
    ...
    async def copyright_files(self, filename: str) -> list[dict]:
        ...
        try:
            md_count = sum(
                ...
                if f.is_file() and f.suffix.lower() == ".md" and f.name.lower() not in SIDECAR_MD
            )
        except OSError:
            md_count = 1
        ...
        for f in sorted(song_dir.iterdir()):
            ...
            if md_count > 1:
                ...
                if not (song_stem and song_stem in name_l or title_first and title_first in name_l):
                    continue
            try:
                rel = str(f.relative_to(vault)).replace("\\", "/") if vault else f.name
                out.append(
            ...
            if dir_key not in dir_cache:
                ...
                try:
                    for f in full.parent.iterdir():
                        ...
                        name_l = f.name.lower()
                        is_cover = (
                            "cover" in name_l and "vertical" not in name_l and "wide" not in name_l
                        )
                        is_screenshot = "screenshot" in name_l
                        if not (is_cover or is_screenshot):
                            continue
