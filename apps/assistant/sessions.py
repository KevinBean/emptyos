...
class SessionsMixin:
    ...
    def _init_db(self):
        ...
        if json_path.exists() and self.db.execute("SELECT COUNT(*) FROM sessions").fetchone()[0] == 0:
        ...
        try:
            ...
            for sid, s in data.items():
                self.db.execute(
                    ...
                    (sid, s.get("name", "New chat"), s.get("backend", "auto"),
                     s.get("system_prompt", ""), s.get("created", "")),
                )
                for msg in s.get("messages", []):
                    self.db.execute(
                        "INSERT INTO messages (session_id, role, text, agent, ts) VALUES (?,?,?,?,?)",
                        (sid, msg.get("role", ""), msg.get("text", ""),
                         msg.get("agent", ""), msg.get("ts", "")),
                    )
            self.db.commit()
    ...
    def _get_session(self, sid: str) -> dict | None:
        ...
        return {
            "id": row["id"], "name": row["name"], "backend": row["backend"],
            "system_prompt": row["system_prompt"], "messages": messages,
            "created": row["created"],
        }
    ...
    def _create_session(self, name: str = "", backend: str = "auto") -> dict:
        ...
        self.db.commit()
        return {"id": sid, "name": name or "New chat", "backend": backend,
                "system_prompt": "", "messages": [], "created": created}
    ...
    async def _auto_name(self, session_id: str, first_message: str, websocket=None):
        ...
        try:
            name = await self.think(
                f"Summarize this chat topic in 3-5 words (no quotes, no punctuation):\n{first_message[:200]}",
    ...
    async def _export_session(self, session_id: str) -> dict:
        ...
        ts = session.get("created", datetime.now(timezone.utc).isoformat())[:10]
        lines = [
            "---",
            f"date: {ts}",
            "tags:",
        ...
        for msg in messages:
            role = "**You**" if msg["role"] == "user" else f"**AI** ({msg.get('agent', 'assistant')})"
            text = msg.get("text", "")
            lines.append(f"{role}:")
            lines.append(text)
            lines.append("")
        ...
        vault_dir = self.vault_config_path("exports", "30_Resources/EmptyOS/assistant")  # default fallback
        if not vault_dir:
